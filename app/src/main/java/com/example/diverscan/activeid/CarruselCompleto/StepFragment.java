...
public class StepFragment extends StepView {
    ...
    private void initData() {
        ...
        if (imageView != null) {
            byte[] decodedString = Base64.decode(step.getDrawable(), Base64.DEFAULT);
    ...
    public void UpdateFragment(String fotoBase, int position){

        byte[] decodedString = Base64.decode(fotoBase, Base64.DEFAULT);
        Bitmap decodedByte = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
        imageView.setImageBitmap(decodedByte);
