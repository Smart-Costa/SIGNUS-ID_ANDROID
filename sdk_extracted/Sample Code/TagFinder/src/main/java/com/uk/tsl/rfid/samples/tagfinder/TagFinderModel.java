...
public class TagFinderModel extends ModelBase {
    ...
    public void setTargetTagEpc(String targetTagEpc)
    {
        ...
        {
            mTargetTagEpc = targetTagEpc.trim().toUpperCase();
        }
    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!StringHelper.isNullOrEmpty(mTargetTagEpc) && mEnabled)
                    {
                        ...
                        // Series 3 readers can stop as soon as a single tag has been found
                        // Other readers will ignore the unknown parameter
                        mInventoryCommand.setHaltOnRounds(1);

                        // Expect up to 2 matching tags (
                        mInventoryCommand.setQAlgorithm(QAlgorithm.FIXED);
                        mInventoryCommand.setQValue(1);

                        // Get settings for confirmation
                        mInventoryCommand.setReadParameters(TriState.YES);
                    }
