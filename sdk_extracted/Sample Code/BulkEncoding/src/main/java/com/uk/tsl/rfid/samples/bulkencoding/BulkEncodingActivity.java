...
import com.uk.tsl.rfid.samples.bulkencoding.databinding.ActivityBulkEncodingBinding;
...
public class BulkEncodingActivity extends AppCompatActivity
{
    // Debugging
    private static final String TAG = "BEA";
    private static final boolean D = BuildConfig.DEBUG;


    private AppBarConfiguration appBarConfiguration;
    private ActivityBulkEncodingBinding binding;
    ...
    /**
     * @return true if the Reader will not disconnect when Activity paused
     */
    public boolean isDisconnectionSuspended()
    {
        return isDisconnectionSuspended;
    }
    /**
     * @param disconnectionSuspended true if the Reader will not disconnect when Activity paused
     */
    public void setDisconnectionSuspended(boolean disconnectionSuspended)
    {
        isDisconnectionSuspended = disconnectionSuspended;
    }
    private boolean isDisconnectionSuspended = false;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        // BLE is disabled by default as it requires a dependency library (Blessed)
        ReaderManager.sharedInstance().setBleSupportEnabled(true);
        ...
        // Now build the UI
        binding = ActivityBulkEncodingBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        setSupportActionBar(binding.toolbar);

        NavController navController = Navigation.findNavController(this, R.id.nav_host_fragment_content_bulk_encoding);
    ...
    public synchronized void onPause() {
        ...
        if( !isDisconnectionSuspended() && !ReaderManager.sharedInstance().didCauseOnPause() && mReader != null )
        {
            mReader.disconnect();
        }
    ...
    public synchronized void onResume() {
        ...
        setDisconnectionSuspended(false);
    }
...
//            editor.apply();
            ...
            // Only need to optimise comms if using the ePop-Loq connector as it defaults to 115200 for backwards compatibility.
        ...
        if( reader != null )
        {
            ...
            if (transport.type() == TransportType.USB)
            {
                ...
                if (reader.getDeviceProperties().getInformationCommand().getAsciiProtocol().startsWith("3"))
                {
                    try
                    {
                        Log.d(TAG, String.format("Changing comm speed..."));
                        ...
                        if( sCommand.isSuccessful() )
                        {
                            ...
                            {
                                Log.d(TAG, String.format("Baud rate: " + commSpeed));
                            }
                            else
                            {
                                ...
                                reader.disconnect();
                                isConnected = false;
                                Log.d(TAG, String.format("!!! Unable to change Baud rate - disconnecting... !!!"));
                                Log.d(TAG, String.format("!!! Physically disconnect the ePop-Loq device. !!!"));
                            }
                        }
                        else
                        {
                            Log.d(TAG, String.format("!!! Unsupported Baud rate (%d) !!!", commSpeed));
                        }
                    }
                    catch(Exception e)
                    {
                        Log.d(TAG, String.format("!!! Unsupported Baud rate - ignoring... !!!"));
                    }
                }
            }
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        ...
        else if (id == R.id.connect_reader_menu_item)
        {
            ...
            setDisconnectionSuspended(true);
            int index = -1;
            if( mReader != null )
            {
    ...
    private String mPermission1;
    private String mPermission2;
    //private String mPermission3;

    private void determineRequiredBluetoothPermissions()
    {
        //mPermission3 = null;
        if (Build.VERSION.SDK_INT < 29)
        {
            mPermission1 = Manifest.permission.BLUETOOTH;
            mPermission2 = Manifest.permission.ACCESS_COARSE_LOCATION;

            bluetoothPermissions = new String[]{mPermission1, mPermission2};
        }
        else if (Build.VERSION.SDK_INT < 31)
        {
            mPermission1 = Manifest.permission.BLUETOOTH;
            mPermission2 = Manifest.permission.ACCESS_FINE_LOCATION;

            bluetoothPermissions = new String[]{mPermission1, mPermission2};
        }
        else
        {
            mPermission1 = Manifest.permission.BLUETOOTH_CONNECT;
            mPermission2 = Manifest.permission.BLUETOOTH_SCAN;
    ...
    {
        determineRequiredBluetoothPermissions();

        if (ContextCompat.checkSelfPermission(this, mPermission1) != PackageManager.PERMISSION_GRANTED
                || ContextCompat.checkSelfPermission(this, mPermission2) != PackageManager.PERMISSION_GRANTED
            //|| mPermission3 != null && ContextCompat.checkSelfPermission(this, mPermission3) != PackageManager.PERMISSION_GRANTED
        )
        {
            binding.bluetoothPermissionsPrompt.setVisibility(View.VISIBLE);
            if (shouldShowRequestPermissionRationale(mPermission1))
            {
    ...
    private String[] bluetoothPermissions = new String[]{Manifest.permission.BLUETOOTH_CONNECT, Manifest.permission.BLUETOOTH_SCAN};
